...
describe('buildArduinoCliCompileArgs', () => {
  ...
  it('appends local board package libraries before export args', () => {
    const args = buildArduinoCliCompileArgs(
      { platform: 'STMicroelectronics:stm32:Nucleo_144:pnum=NUCLEO_H563ZI' },
      {
        sketchPath: '/work/examples/Baremetal/Baremetal.ino',
        libraryPath: '/work/src',
        localLibraryPaths: ['/packages/com.openplc.stm32-wp/hal/arduino/libraries'],
        parallel: false,
      },
    )
    expect(args.filter((a) => a === '--library')).toHaveLength(2)
    expect(args).toContain('/packages/com.openplc.stm32-wp/hal/arduino/libraries')
    expect(args.indexOf('/packages/com.openplc.stm32-wp/hal/arduino/libraries')).toBeLessThan(
      args.indexOf('--export-binaries'),
    )
  })
