...
export interface BoardHalsBuildEntry extends BoardHalsCompileEntry {
  ...
  /** Package-supplied Arduino libraries directory from VPP `device.hal.libraries`.
   *  Passed to arduino-cli as an additional `--library` path so optional
   *  board features such as LCD drivers can travel with the package. */
...
async function runCompilePipelineInner(
  ...
  const arduinoArgs = buildArduinoCliCompileArgs(boardEntry, {
    ...
    ...(boardEntry.localLibrariesDir ? { localLibraryPaths: [boardEntry.localLibrariesDir] } : {}),
  })
